    ...
    <SafeAreaView className="flex-1 bg-slate-950">
      ...
      <ScrollView className="px-6" showsVerticalScrollIndicator={false}>
        {history.length === 0 ? (
          ...
          history.map((item) => (
            <View key={item.id} className="bg-slate-900 rounded-3xl overflow-hidden border border-slate-800 mb-6">
              <Image source={{ uri: item.imageUri }} style={{ width: '100%', height: 200 }} className="opacity-90" />
              <View className="p-5 relative">
                <Text className="text-slate-400 text-xs font-bold uppercase tracking-widest">{item.date}</Text>
                
                <View className="flex-row justify-between items-start mt-2 mb-4">
                  <View>
                    <Text className="text-white font-extrabold text-xl">{item.disease}</Text>
                  </View>
                  <View className="bg-emerald-500/10 px-3 py-1.5 rounded-xl border border-emerald-500/20">
                    <Text className="text-emerald-400 font-bold text-xs uppercase">{item.confidence}</Text>
                  </View>
                </View>
